...
sequenceDiagram
    ...
    participant CSR as csr_generator.py
    participant CA as ca_setup.py
    participant Issuer as cert_issuer.py
    participant Audit as audit_log.py
    participant Disk as storage/

    User->>CLI: python main.py issue --name "Alice" --email alice@example.com
    CLI->>CA: initialize() — load CA key + cert
    CA->>Disk: read ca_private_key.pem + ca_certificate.pem
    CLI->>CSR: CSRGenerator(subject)
    CSR->>CSR: generate_rsa_key() — RSA-2048
    CSR->>CSR: build_csr(key) — PKCS#10, signed SHA-256
    CSR->>Disk: save alice_private_key.pem + alice_csr.pem
    CLI->>Issuer: issue(csr, validity_days=365)
    Issuer->>Issuer: verify CSR signature
    Issuer->>Issuer: build X.509 v3 cert — signed with CA key (SHA-256)
    Issuer->>Disk: save alice_<serial>.pem
    Issuer->>Audit: log("CERT_ISSUED", {...})
    Audit->>Disk: append to audit_log.json (hash-chained)
    CLI->>User: print cert details + serial number
```
...
sequenceDiagram
    actor User
    participant CLI as cli.py
    participant Verifier as cert_verifier.py
    participant CRL as crl_manager.py
    participant Audit as audit_log.py

    User->>CLI: python main.py verify --cert storage/certs/alice_<serial>.pem
    CLI->>Verifier: verify(cert)

    Verifier->>Verifier: CHECK 1 — Signature<br/>CA public key verifies cert signature (PKCS1v15 + SHA-256)
    alt Signature invalid
        Verifier-->>User: INVALID — signature verification failed
    end

    Verifier->>Verifier: CHECK 2 — Expiry<br/>not_valid_before <= now <= not_valid_after
    alt Certificate expired
        Verifier-->>User: INVALID — certificate has expired
    end

    Verifier->>CRL: is_revoked(serial)
    CRL->>CRL: O(1) lookup in revoked registry
    alt Serial is revoked
        Verifier-->>User: INVALID — revoked (reason + timestamp)
    end

    Verifier->>Audit: log("CERT_VERIFIED", {valid: true, checks: {...}})
    Verifier-->>User: VALID — all checks passed
```
...
sequenceDiagram
    ...
    participant CRL as crl_manager.py
    participant Audit as audit_log.py
    participant Disk as storage/crl/

    User->>CLI: python main.py revoke --serial <n> --reason key_compromise
    CLI->>CRL: revoke(serial, reason)
    CRL->>CRL: validate reason string
    CRL->>CRL: check not already revoked
    CRL->>CRL: add to in-memory registry
    CRL->>Disk: save revoked_registry.json
    CRL->>CRL: build_crl() — X.509 CRL signed with CA key (SHA-256)
    CRL->>Disk: save ca.crl.pem
    CRL->>Audit: log("CERT_REVOKED", {serial, reason, revoked_at})
    Audit->>Disk: append to audit_log.json (hash-chained)
    CLI-->>User: [OK] Serial revoked
```
