export default function Guide() {
  return (
    <div style={{ maxWidth: '860px' }}>
      ...
      {/* Real world analogy */}
      <Section icon="🌍" title="Real World Example">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginTop: '1rem' }}>
          {[
            { icon: '🏢', title: 'Small Company', desc: 'Issue certificates to employees for VPN access, email signing, internal websites' },
            { icon: '🎓', title: 'Learning', desc: 'Understand how HTTPS, digital signatures, and PKI actually work under the hood' },
            { icon: '🔬', title: 'Research', desc: 'Study certificate lifecycle management, automation, and PKI usability' },
          ].map(c => (
      ...
      {/* Key concepts */}
      <Section icon="📚" title="Key Concepts (Plain English)">
        {[
      ...
      {/* Getting started */}
      <Section icon="🚀" title="Getting Started — Step by Step">
        ...
        {[
          {
            step: 1, label: 'Initialise the CA', tab: 'Init CA',
            desc: 'Go to Init CA in the sidebar and click the Init button. This creates your root Certificate Authority — the master key that signs everything. You only do this once. If a CA already exists, it just loads it.',
          },
          {
            step: 2, label: 'Issue a Certificate', tab: 'Issue Cert',
            desc: 'Go to Issue Cert. Fill in a name, email, organisation, and country (2-letter code like IN, US, GB). Choose a template — use client_auth for a person, tls_server for a website. Click Issue Certificate. You\'ll get a PEM certificate and a serial number.',
          },
          {
            step: 3, label: 'View Your Certificates', tab: 'Certificates',
            desc: 'Go to Certificates to see all issued certificates in a table. You can see who they were issued to, when they expire, and whether they\'re active or revoked.',
          },
          {
            step: 4, label: 'Verify a Certificate', tab: 'Verify',
            desc: 'Go to Verify. Upload the .pem certificate file. The system checks: Is the signature valid? Has it expired? Has it been revoked? You\'ll get a clear Valid or Invalid result with reasons.',
          },
          {
            step: 5, label: 'Revoke a Certificate', tab: 'Revoke',
            desc: 'If someone leaves the company or a certificate is compromised, go to Revoke, paste the serial number, choose a reason, and click Revoke. The certificate is immediately cancelled and added to the CRL.',
          },
          {
            step: 6, label: 'Set Up Automation', tab: 'Policy Manager',
            desc: 'Go to Policy Manager and create a policy for client_auth. Set max validity to 365 days, turn on Auto-Renew, set "Renew 30 days before expiry". Now the system will automatically renew certificates before they expire — no manual work needed.',
          },
      ...
      {/* Certificate types */}
      <Section icon="📋" title="Certificate Types — Which One to Use?">
        {[
      ...
      {/* Self managing */}
      <Section icon="⚙️" title="Self-Managing PKI — What Makes This Different">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {[
            { icon: '🔄', title: 'Auto-Renewal', desc: 'Set a policy saying "renew 30 days before expiry". The system checks daily and automatically issues a new certificate, revoking the old one.' },
            { icon: '⏳', title: 'Expiry Warnings', desc: 'The dashboard shows all certificates expiring soon. Filter by 7, 14, 30, 60, or 90 days ahead.' },
            { icon: '📝', title: 'Self-Service Requests', desc: 'Employees request certificates themselves by filling a simple form. No PKI knowledge needed. Admin approves or rejects from a queue.' },
            { icon: '✅', title: 'Approval Policies', desc: 'Set per-template rules. Code signing certificates require admin approval. Client auth certificates are auto-approved. You control the rules.' },
          ].map(({ icon, title, desc }) => (
